    ...
    for sim_fn in similarity_fns:
        for character in character_names:
            if characetrname_to_id[character] not in notable_names_index:
                continue
            # print("%s : %s" % (sim_fn.__qualname__, character))
            ranks, sims = rank_notable(characetrname_to_id[character], term_character_matrix, sim_fn, notable_names_index, True)
    ...
    for sim_fn in similarity_fns:
        for character in character_names:
            if characetrname_to_id[character] not in notable_names_index:
                continue
            # print("%s : %s" % (sim_fn.__qualname__, character))
            ranks, sims = rank_notable(characetrname_to_id[character], term_character_matrix, sim_fn, notable_names_index, False)
            if sims[0] < similarity:
                similarity = sims[0]
                character1 = character
                character2 = character_names[ranks[0]]
...
def male_and_female(term_character_matrix, similarity_fn, male, female, character_names):
    ...
    # print(np.sum(similarities_m_m))
    # print(np.sum(similarities_m_m) / len(similarities_m_m))
...
#    print(least)
    ...
    #
    # for sim_fn in similarity_fns:
    #     print('\nAverage and standard deviation of similarities between female and male characters with %s'
    #           %(sim_fn.__qualname__))
